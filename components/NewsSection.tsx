...
const NewsSection: React.FC<NewsSectionProps> = ({ news, isLoading, articles = [], isLoadingMore, onLoadMore, showLoadMore, onVisible }) => {
  ...
  return (
    <div ref={sectionRef} className="space-y-20">
      ...
      <div className="glass-card p-12 md:p-16 rounded-[4rem] border border-white/10 bg-gradient-to-br from-slate-900/60 to-slate-950/40 relative overflow-hidden group/featured">
        <div className="relative z-10 max-w-5xl">
          <h3 className="text-[11px] font-black text-slate-500 uppercase tracking-[0.5em] mb-10 italic">Informativo de Redação</h3>
          {isLoading ? (
            <div className="space-y-8 animate-pulse"><div className="h-8 bg-slate-800 rounded w-3/4"></div><div className="h-8 bg-slate-800 rounded w-full"></div></div>
          ) : news ? (
            <div className="space-y-12">
              <p className="text-3xl md:text-5xl font-black text-white leading-tight italic tracking-tighter drop-shadow-2xl">"{news.text}"</p>
            </div>
          ) : <p className="text-slate-500 font-black uppercase tracking-[0.3em] italic">Conectando ao sistema editorial...</p>}
        </div>
      </div>
